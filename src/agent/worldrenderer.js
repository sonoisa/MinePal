...
import { dispose3 } from 'prismarine-viewer/viewer/lib/dispose.js'
import { loadImage } from 'canvas'

const public_dir = url.fileURLToPath(import.meta.url + "/../../../node_modules/prismarine-viewer/public/")
...
function loadTexture (texture, cb) {
  if (textureCache[texture]) {
    ...
    loadImage(path.resolve(public_dir, texture)).then(image => {
...
function loadJSON (json, cb) {
  const data = readFileSync(path.resolve(public_dir, json), 'utf8');
  cb(JSON.parse(data))
}
...
export class WorldRenderer {
  constructor (scene, numWorkers = 4) {
    ...
    for (let i = 0; i < numWorkers; i++) {
      ...
      const src = url.fileURLToPath(import.meta.url + "/../../../node_modules/prismarine-viewer/viewer/lib/worker.js");
