...
import _prismarineViewer from 'prismarine-viewer';
const { viewer: prismarineViewer } = _prismarineViewer;


const Viewer = prismarineViewer.Viewer;
const WorldView = prismarineViewer.WorldView;
const getBufferFromStream = prismarineViewer.getBufferFromStream;
...
const CanvasRenderer = function ( parameters ) {
	...
	*/
	...
	function textureToPattern( texture ) {
		...
		var canvas_width = image.width * ( mirrorX ? 2 : 1 );
		var canvas_height = image.height * ( mirrorY ? 2 : 1 );
...
export class Camera {
    constructor(bot) {
        ...
        this.viewer.world = new WorldRenderer(this.viewer.scene);
    }
  
    async init() {
        // bot の座標を基準に少し上へカメラをずらして中心座標に
        const botPos = this.bot.entity.position;
        const center = new Vec3(botPos.x, botPos.y + 1, botPos.z);
    ...
    async takePicture(direction, image_filepath) {
        ...
        await fs.writeFile(image_filepath, buf);
    }
}
